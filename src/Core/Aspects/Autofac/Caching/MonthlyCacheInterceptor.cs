...
{
    public class MonthlyCacheInterceptor : InterceptorBase<MonthlyCacheAttribute>
    {
        private readonly ICacheManager _cacheManager;
        private readonly IHttpContextAccessor _httpContext;

        public MonthlyCacheInterceptor()
        {
        ...
        private MonthlyCacheAttribute GetAttribute(MethodInfo methodInfo, Type type)
        {
            var attribute = methodInfo.GetCustomAttribute<MonthlyCacheAttribute>(true);
            if (attribute is not null)
                return attribute;
            return type.GetTypeInfo().GetCustomAttribute<MonthlyCacheAttribute>(true);
        }
    }
}
