...
{
    public class YearlyCacheInterceptor : InterceptorBase<YearlyCacheAttribute>
    {
        private readonly ICacheManager _cacheManager;
        private readonly IHttpContextAccessor _httpContext;

        public YearlyCacheInterceptor()
        {
        ...
        public override void Intercept(IInvocation invocation)
        {
            ...
            _cacheManager.Remove(allKey);
            _cacheManager.Add(allKey, allRequests, attribute._duration);

            _cacheManager.Add(key, invocation.ReturnValue, attribute._duration);
        }

        private YearlyCacheAttribute GetAttribute(MethodInfo methodInfo, Type type)
        {
            var attribute = methodInfo.GetCustomAttribute<YearlyCacheAttribute>(true);
            if (attribute is not null)
                return attribute;
            return type.GetTypeInfo().GetCustomAttribute<YearlyCacheAttribute>(true);
        }
    }
}
